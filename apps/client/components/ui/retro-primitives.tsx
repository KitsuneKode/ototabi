 ...
 */
export function MonoLabel({ children, className, as: Tag = 'span', htmlFor, ...props }: MonoLabelProps) {
  return (
    <Tag
      className={cn('font-mono text-[10px] font-bold uppercase tracking-widest text-muted-foreground', className)}
      {...(htmlFor ? { htmlFor } : {})}
      {...props}
    >
      {children}
...
export function StatusBadge({ children, variant = 'default', className }: StatusBadgeProps) {
  return (
    <span
      className={cn(
        'inline-flex items-center gap-1.5 border px-2 py-0.5 rounded-full font-mono text-[9px] font-bold uppercase tracking-wider',
 ...
 */
export function MechButton({ children, className, variant = 'default', ...props }: MechButtonProps) {
  return (
    <button
      className={cn(
        'btn-mechanical inline-flex items-center justify-center gap-2 rounded px-4 py-2 font-bold uppercase tracking-widest text-secondary-foreground text-xs',
        variant === 'danger' && 'bg-gradient-to-b from-destructive/80 to-destructive border-destructive/60 text-destructive-foreground shadow-none hover:brightness-110',
        className,
      )}
      {...props}
    >
